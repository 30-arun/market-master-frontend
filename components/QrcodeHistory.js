	...
	const fetchQRCodeHistory = async () => {
		...
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/qrcode-history/?user_id=${userId}`
			);
		...
		try {
			...
			if (confirmed.isConfirmed) {
				const response = await axios.delete(
					`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/qrcode-history/?user_id=${userId}&qrcode_id=${id}`
				);
