		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/user-template/${userId}/`
				);
		...
		try {
			...
			if (result.isConfirmed) {
				const response = await axios.delete(
					`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/user-template-detail/${userId}/${id}/`
				);
							...
							<div className="card h-100 template-card">
								<Link href={`/mysite/${template.id}/website`}>
									<img
										src={
											template.image
												? `${process.env.NEXT_PUBLIC_API_DOMAIN_NAME}${template.image}`
										...
										{template.ecommerce ? (
											<>
												<span
													...
													onClick={() =>
														handleQrCode(
															`${process.env.NEXT_PUBLIC_API_PROTOCOL}://${template.slug}.${process.env.NEXT_PUBLIC_API_SITE_NAME}/`
														)
													}
												>
