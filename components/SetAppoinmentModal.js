...
const SetAppointmentModal = ({ show, handleClose, templateId }) => {
	...
	const fetchSubmittedDates = async () => {
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/available-time/?template_id=${templateId}`
	...
	const handleAdminSubmit = async (e) => {
		...
		try {
			...
			await axios.post(
				`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/available-time/`,
				{
	...
	const handleDelete = async (id) => {
		try {
			...
			if (result.isConfirmed) {
				await axios.delete(
					`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/available-time/?template_id=${templateId}&available_time_id=${id}`
				);
