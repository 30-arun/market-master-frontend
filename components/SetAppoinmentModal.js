...
const SetAppointmentModal = ({ show, handleClose, templateId }) => {
  ...
  const fetchSubmittedDates = async () => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/store/available-time/?template_id=${templateId}`
  ...
  const handleAdminSubmit = async (e) => {
    ...
    try {
      ...
      const startTimeFormatted = formatTimeForBackend(adminData.startTime);
      const endTimeFormatted = formatTimeForBackend(adminData.endTime);

      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/store/available-time/`,
        {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/store/available-time/?template_id=${templateId}&available_time_id=${id}`
        );
  ...
  return (
    <Modal show={show} onHide={handleClose} keyboard={false}>
      ...
      <Modal.Body>
        ...
        >
          <Nav variant="tabs" className="mb-3 justify-content-center">
            <Nav.Item>
              <Nav.Link eventKey="setAppointment">Set Appointment</Nav.Link>
            </Nav.Item>
            <Nav.Item>
              ...
              <div className="container">
                <div className="row justify-content-center">
                  <div className="col-12">
                    <form onSubmit={handleAdminSubmit} id="setAppoinenForm">
                      <div className="mb-3 d-flex justify-content-center">
                        <Calendar
                          ...
                          tileClassName={({ date, view }) => {
                            if (
                              view === "month" &&
                              fetchedDates.includes(date.toDateString())
                            ) {
      ...
      <Modal.Footer>
        ...
        {key === "setAppointment" && (
          <button type="submit" className="btn btn-dark" form="setAppoinenForm">
            Set Availability
          </button>
        )}
      </Modal.Footer>
