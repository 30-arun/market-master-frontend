		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.NEXT_PUBLIC_API_URL}/store/booking/?template_id=${templateId}`
				);
	...
	const handleDeleteMessage = async (id) => {
		try {
			...
			if (result.isConfirmed) {
				await axios.delete(
					`${process.env.NEXT_PUBLIC_API_URL}/store/booking/?template_id=${templateId}&booking_id=${id}`
				);
