...
function MessagesModal({ show, handleClose, templateId }) {
	...
	const fetchData = async () => {
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/contact/?template_id=${templateId}`
			);
	...
	const handleDeleteMessage = async (id) => {
		try {
			...
			if (result.isConfirmed) {
				await axios.delete(
					`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/contact/?template_id=${templateId}&contact_id=${id}`
				);
	...
	const sendReply = async (id) => {
		...
		try {
			...
			await axios.post(
				`${process.env.NEXT_PUBLIC_API_ROUTE_NAME}/store/contact-replied/`,
				{
					contact_id: id,
