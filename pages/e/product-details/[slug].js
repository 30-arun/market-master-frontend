...
function ProductDetails() {
	...
	const _productDetailsReducer = useSelector(
	...
	useEffect(() => {
		...
		axios
			...
			.then((response) => {
				dispatch(productDetails(slug, response.data.id));
				setpLoading(false);
			})
			.catch((error) => {
				console.log(error);
			});
	}, [dispatch, slug, router.isReady]);

	const addToCart = () => {
		// console.log(qty)

		router.push(`/e/cart/${slug}?qty=${qty}`);
	};
						...
						<div className="pt-6 ">
							<div className="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
								<div className="aspect-h-4 aspect-w-3  overflow-hidden rounded-lg lg:block">
									<img
										src={`${process.env.NEXT_PUBLIC_API_DOMAIN_NAME}${product.image}`}
								...
								<div className="aspect-h-5 aspect-w-4 lg:aspect-h-4 lg:aspect-w-3 sm:overflow-hidden sm:rounded-lg">
									<div className="mt-4 lg:row-span-3 lg:mt-0">
										...
										<p className="text-3xl tracking-tight text-cyan-800">
											${product.price}
										</p>
										<div className="mt-6">
