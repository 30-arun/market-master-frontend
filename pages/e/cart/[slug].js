...
function Cart() {
	...
	const { main, id, slug } = router.query;
	...
	useEffect(() => {
		axios
			...
			.then((response) => {
				if (slug !== "open" && slug) {
					dispatch(addToCart(slug, qty, response.data.id));
				}
			})
			.catch((error) => {
				console.log(error);
	...
	return (
		<>
			...
			<div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-10 lg:max-w-7xl">
				<h2 className="text-2xl font-bold tracking-tight text-cyan-800 mb-10 lg:mb-0">
					Shopping Cart
				</h2>
