...
const BookingSystem = () => {
  ...
  const handleAdminSubmit = async (e) => {
    e.preventDefault();
    try {
      const formattedDate = adminData.selectedDate.toISOString().split("T")[0];
      const timesArray = adminData.times.split(",").map((time) => time.trim());
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/store/available-time/?template_id=134`
      )
      .then((response) => {
